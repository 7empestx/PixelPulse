...
`mcp_server.py` is an MCP server that exposes the PixelPulse REST API as tools Claude Code can call directly. It connects to the device at `$PIXELPULSE_URL` (default: `http://pixelpulse.local`). Registered in `.mcp.json` and loads automatically in Claude Code.
